...
assert.deepEqual(policy, { mode: "SHADOW", go_basis_points: 1500, go_xl_basis_points: 1500 });
...
for (const fixture of fixtures) {
  ...
  assert.equal(ok(await db.from("driver_wallet_transactions").select("id").eq("trip_id", fixture.tripId).eq("tx_type", "commission"), "Legacy commission count").length, 1);
}
...
console.log("PHASE 2 DURABLE SHADOW RECOVERY DISPOSABLE E2E PASSED");
