-- Disposable only. Release package includes this function inline.
begin;
...
declare a public.phase4_fee_assessments;l public.phase4_customer_liabilities;
  c public.phase4_driver_compensations;v_original uuid;v_reversal uuid;v_result jsonb;v_now timestamptz;
begin
  ...
  select financial_transaction_id into v_reversal from public.phase4_financial_actions
    where source_key='phase4:reversal:'||a.id;
  if found then return jsonb_build_object('assessment_id',a.id,'reversal_transaction_id',v_reversal,'replayed',true);end if;
...
revoke all on function public.phase4b_reverse_unpaid_assessment(uuid,uuid,text) from public,anon,authenticated,service_role;
grant execute on function public.phase4b_reverse_unpaid_assessment(uuid,uuid,text) to service_role;
commit;
