-- MOOVU 9.5 percent commission backfill
-- Review-only migration. Do not run until approved and never run blindly against production.
--
-- Purpose:
-- - Update only completed trips to the current 9.5 percent MOOVU commission rate.
...
-- - Does not create missing wallet transactions.
-- - Does not create missing driver wallets.

begin;

create temporary table moovu_commission_95_backfill as
select
  t.id as trip_id,
  t.driver_id,
  round(coalesce(t.fare_amount, 0)::numeric, 2) as fare_amount,
  9.5::numeric as new_commission_pct,
  round((coalesce(t.fare_amount, 0)::numeric * 0.095), 2) as new_commission_amount,
  round(
    coalesce(t.fare_amount, 0)::numeric
    - round((coalesce(t.fare_amount, 0)::numeric * 0.095), 2),
    2
...
where t.status = 'completed'
  ...
  and (
    coalesce(t.commission_pct::numeric, -1) <> 9.5
    or coalesce(round(t.commission_amount::numeric, 2), -1) <> round((coalesce(t.fare_amount, 0)::numeric * 0.095), 2)
...
from moovu_commission_95_backfill;
...
-- Recalculate existing driver wallets from completed trips and approved/recorded commission settlements.
...
set
  ...
  account_status = case when v.balance_due >= 100 then 'locked' when v.balance_due > 0 then 'due' else 'settled' end,
  updated_at = now()
from wallet_values v
where w.driver_id = v.driver_id;

-- Optional hardening to review before applying. Uncomment only after confirming there are no duplicates.
-- create unique index concurrently if not exists driver_wallet_transactions_one_commission_per_trip
--   on public.driver_wallet_transactions (trip_id)
--   where tx_type = 'commission' and trip_id is not null;
--
-- create unique index concurrently if not exists driver_payment_requests_unique_reference
--   on public.driver_payment_requests (payment_reference)
