...
export async function POST(req: Request) {
  try {
    ...
    // Find driver_id for this auth user
    ...
    if (dErr || !driver) return NextResponse.json({ ok: false, error: "Driver not found" }, { status: 404 });
    ...
    const { error: upErr } = await supabaseAdmin.from("drivers").update({ online: wantOnline }).eq("id", driverId);
