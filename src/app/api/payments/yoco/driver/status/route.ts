...
export async function GET(req: Request) {
  ...
  const { data: attempt, error } = await auth.supabaseAdmin.from("driver_online_payment_attempts")
    .select("state,amount_cents,currency,reconciliation_state,verified_at,obligation_type")
  ...
  return NextResponse.json({ ok: true, payment: {
    ...
    context: "MOOVU commission payment",
  } });
}
