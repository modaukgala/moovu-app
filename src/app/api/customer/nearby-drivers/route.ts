...
export async function GET(req: Request) {
  ...
  const cacheKey = `nearby:${auth.customer.id}:${rideOption}:${lat.toFixed(4)}:${lng.toFixed(4)}`;
  const { value, cacheStatus } = await resolveCachedJson(cacheKey, 5000, async () => {
    const now = Date.now();
    ...
    if (driverResult.error) {
      ...
      throw new Error("Nearby drivers are temporarily unavailable.");
    }
